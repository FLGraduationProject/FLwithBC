...
def divideData2Clients(local_data_ratio, batch_size, n_clients, eq_IID=False, dif_IID=False, Non_IID=False):
    ...
    if eq_IID:
        ...
        traindata_split = torch.utils.data.random_split(trainset, np.random.multinomial(trainset.data.shape[0]-n_clients, np.ones(n_clients)/n_clients)+1)
        train_loader = [torch.utils.data.DataLoader(x, batch_size=batch_size, shuffle=True) for x in traindata_split]
        ...
        each_parti_data_size = (np.random.multinomial(len(sorted_inds)-n_clients, np.ones(n_clients)/n_clients)+1).tolist()
