...
def code_worker(code_sequence, clientIDs, workQ, resultQs):
...
def gpu_worker(clientIDs, client_model_types, clientLoaders, testLoader, workQ, resultQs, device):

  client_models = {clientID: nn.DataParallel(client_model_types[clientID]()).cuda() for clientID in clientIDs}
  ...
  while not processDone:
    if not workQ.empty():
      ...
      if msg['train_method'] == 'KD_train':
        ...
        client_model.load_state_dict({k: v.cuda() for k, v in msg['model_data']['main_client'].items()})
        teacher_models = []
        for teacher in msg['model_data']['teacher_clients'].keys():
          teacher_model = client_models[teacher]
          teacher_model.load_state_dict({k: v.cuda() for k, v in msg['model_data']['teacher_clients'][teacher].items()})
          teacher_models.append(teacher_model)
        updated_params, test_result = cf.KD_trainNtest(client_model, msg['client'], clientLoaders[msg['client']], testLoader, teacher_models, device)
